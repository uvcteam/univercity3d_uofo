...
public class UICenterOnChild : MonoBehaviour
{
    ...
    public void Recenter(GameObject objectToCenterOn)
    {
        if (mDrag == null)
        {
            mDrag = NGUITools.FindInParents<UIDraggablePanel>(gameObject);
        ...
        mCenteredObject = objectToCenterOn;

        // Figure out the difference between the chosen child and the panel's center in local coordinates
        Vector3 cp = dt.InverseTransformPoint(objectToCenterOn.transform.position);
        Vector3 cc = dt.InverseTransformPoint(center);
        Vector3 offset = cp - cc;
